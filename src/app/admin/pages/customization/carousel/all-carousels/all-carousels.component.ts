...
@Component({
  selector: 'app-all-carousels',
  templateUrl: './all-carousels.component.html',
  styleUrls: ['./all-carousels.component.scss']
})
export class AllCarouselsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.subForm = formValue.pipe(
      ...
      .subscribe(res => {
        this.searchCarousels = res.data;
        this.carousels = this.searchCarousels;
  ...
  filterData(value: any, index: number, type: string) {
    switch (type) {
      ...
      case 'carousel': {
        this.filter = {...this.filter, ...{carousel: value}};
  ...
  private getAllCarousels() {
    ...
    this.subDataOne = this.carouselService.getAllCarousels(filterData, this.searchQuery)
      .subscribe(res => {
        this.spinner.hide();
        this.carousels = res.data;
        if (this.carousels && this.carousels.length) {
          this.carousels.forEach((m, i) => {
            const index = this.selectedIds.findIndex(f => f === m._id);
            this.carousels[i].select = index !== -1;
          });

          this.totalCarousels = res.count;
          if (!this.searchQuery) {
            this.holdPrevData = res.data;
            this.totalCarouselsStore = res.count;
          }
  ...
  private deleteCarouselById(id: string) {
    this.spinner.show();
    this.subDataFive = this.carouselService.deleteCarouselById(id, false)
      .subscribe(res => {
        this.spinner.hide();
        if (res.success) {
  ...
  private deleteMultipleCarouselById() {
    this.spinner.show();
    this.subDataFour = this.carouselService.deleteMultipleCarouselById(this.selectedIds, true)
      .subscribe(res => {
        this.spinner.hide();
        if (res.success) {
          this.selectedIds = [];
  ...
  private updateMultipleCarouselById(data: any) {
    this.spinner.show();
    this.subDataThree = this.carouselService.updateMultipleCarouselById(this.selectedIds, data)
      .subscribe(res => {
        this.spinner.hide();
        if (res.success) {
          this.selectedIds = [];
   ...
   */
     public openConfirmDialog(type: string, data?: any) {
      if (type === 'delete') {
        ...
        dialogRef.afterClosed().subscribe(dialogResult => {
          if (dialogResult) {
            this.updateMultipleCarouselById(data);
          }
        });
